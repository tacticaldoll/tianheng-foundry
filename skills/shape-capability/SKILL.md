...
- Read [gap-classification.md](references/gap-classification.md) before declaring a capability gap.
- Read [observation-contract.md](references/observation-contract.md) when shaping the measure and
  reaction.
...
These references target the supported Tianheng line declared in `compatibility.json`. Inspect the adopter's resolved version and Tianheng's
...
In a mixed-language repository, narrow the claim to its Rust-observable portion. A constraint whose
meaning depends on non-Rust code is not automatically a Tianheng capability gap.

Do not write adopter Constitution code, placeholder modules, baselines, or generated projection
...
1. Resolve the adopter's Tianheng version from Cargo metadata or lockfile.
2. Inspect that version's public boundary surface, docs, recipe index, and relevant source.
3. Restate the claim as one contradicted structural fact.
4. Test whether an existing static, semantic, runtime, or composed observation already sees it.
5. Record why the nearest recipes fail without overstating their non-observations.

Classify:

- `EXISTING_RECIPE`: route back to `forge-law`;
- `COMPATIBILITY_UNKNOWN`: investigate the resolved/newer Tianheng line first;
- `NON_STRUCTURAL`: keep the claim in process, judgment, history, or API prose;
- `CROSS_LANGUAGE`: narrow to Rust or route to another governance system;
- `TRUE_GAP`: continue.
...
The first two justify capability pressure, not upstream acceptance. Generic best practice and
repository shape remain insufficient.
...
One pressure candidate describes one observation family. Split claims that require unrelated
sources or repair directions.
