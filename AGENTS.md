...
- Does prose retirement delete only a claim an enforced, unbaselined boundary holds whole, and
  refuse a style mandate?
...
Development is linear on `main`. Land each change as a commit on `main` rather than opening a
branch to be integrated later, and keep the history a single lineage. Work that is not ready to
land is not ready to commit.

A second lineage is treated as a hazard rather than a workflow here: this repository has already
lost merged work when `main` diverged from the line those merges landed on, and the loss was
invisible until a gate that had been silently reverted was exercised. One line makes that failure
