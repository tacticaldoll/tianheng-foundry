...
Use when the open question is which code comments accepted law has already made redundant. The
skill reads the canonical projection, inventories structural claims in `//` and `//!` comments, and
deletes only a claim that an **enforced, unbaselined** boundary observes in full — citing that
boundary's identity, rule, severity, and baseline state for every deletion.

Everything else is retained whole and routed: a claim wider than its perimeter or held only by a
`warn` boundary to `amend-law`, a baselined one to `manage-baseline`, an ungoverned one to
`forge-law`, and an unobservable one to `shape-capability`. It never rewrites or narrows a
human-authored sentence, because a narrowed sentence has no proof behind it, and it refuses a
no-comments style mandate outright. Rustdoc on a public item is outside its corpus.

Invoke it explicitly as `$retire-covered-prose`, or let a compatible host select it when asked
which comments existing law has replaced.
...
tests/retirement-scenarios/  prose coverage, retirement, and routing cases
...
Unlike Fornax's own CLI, `th-foundry` deploys from the local checkout rather than resolving a
tagged release. See [`tools/th-foundry-cli/README.md`](tools/th-foundry-cli/README.md) for the
distinction and the plain `hosts`/`status`/`doctor` commands.
...
python3 scripts/test_retirement_scenarios.py
...
`.githooks/pre-commit` runs the network-free part of that gate (the first nine commands)
...
`repair-drift`, `amend-law`, `review-law`, `shape-capability`, `manage-baseline`, and
`retire-covered-prose`. Every skill's `skill.yaml` declares `status: draft` — that is deliberate,
not an oversight: no skill has gone through a documented promotion to `stable`, and this repository
does not yet declare what such a promotion requires. The supported Tianheng line and its checked
representative are declared in
`compatibility.json` and explained in
[`docs/tianheng-compatibility.md`](docs/tianheng-compatibility.md); wider-range coverage is future
...
- **CI-enforced** (`.github/workflows/validate.yml`, on every push and pull request):
  `scripts/validate_skills.py` and the eight `scripts/test_*_scenarios.py` suites, plus a separate
  job that compiles representative generated vocabulary against the real upstream checkout the
  declaration names — asserts, through `cargo metadata`, that the fixture actually resolved every
  Tianheng crate from that checkout rather than a registry copy, and then drives that checkout's
  runner to hold the invocation contract the skills depend on: the `list --format json` projection
  fields, and exit classes `0`, `1`, and `2` from a satisfied, a violated, and an invalid
  declaration. `validate_skills.py` holds
  the distribution structure, manifest consistency, the full `skill.yaml` schema, that every
  directory under `skills/` declares a contract before it ships, that the CLI binding still derives
  its version from `distribution.json`, that the license this repository advertises is the one it
  ships, and that the Tianheng declaration is well formed and still agrees with the fixture, the
  canonical doc, the CI checkout, and every prose surface.
